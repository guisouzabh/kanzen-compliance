...
ALTER TABLE privacy_case_incident_details
  ADD COLUMN comite_id_decisao int NULL AFTER privacy_case_id,
...
CREATE TABLE IF NOT EXISTS privacy_case_decision_approvals (
  ...
  decisao_comunicar_anpd enum('SIM','NAO') NULL,
  decisao_comunicar_titulares enum('SIM','NAO') NULL,
  justificativa text COLLATE utf8mb4_unicode_ci NULL,
  ...
  CONSTRAINT fk_privacy_case_decision_approval_case
    FOREIGN KEY (privacy_case_id) REFERENCES privacy_cases (id) ON DELETE CASCADE,
  CONSTRAINT fk_privacy_case_decision_approval_comite
    FOREIGN KEY (comite_id) REFERENCES comites (id) ON DELETE CASCADE,
  CONSTRAINT fk_privacy_case_decision_approval_usuario
